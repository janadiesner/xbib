...
public class TSVClassifier {

    private String prefix;

    private Map<String,Entry> map;

    public TSVClassifier load(InputStream in, String name, String prefix) throws IOException {
        return load(new InputStreamReader(in, "UTF-8"), name, prefix);
    }

    public TSVClassifier load(Reader reader, String name, String prefix) throws IOException {
    ...
    public Function<String, Entry> mapToKeyValue = (line) -> {
        String[] p = line.split("\t");
        return p.length > 2 ? new Entry(prefix + p[0].trim(), p[1].trim(), p[2].trim()) : null;
    };
