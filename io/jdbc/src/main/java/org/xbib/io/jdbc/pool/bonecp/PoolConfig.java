...
public class PoolConfig implements Cloneable {
     ...
     * <p/>
     * When the available connections are about to run out, new ones are created in batches.
     ...
     * If you do not set this, then a metadata request will be issued instead that should work on all databases but is probably slower.
     ...
     * The specified class will be called according to the connection state (onAcquire, onCheckIn, onCheckout, onDestroy).
     *
    ...
    /**
     * Returns true if pool is configured to create a helper thread to watch over connection acquires that are never released (or released
     * twice).
    ...
    public void setConnectionHookClassName(String connectionHookClassName) {
        ...
        try {
            ...
            this.connectionListener = (ConnectionListener) hookClass;
        } catch (Exception e) {
     ...
     * 1. jdbc-pool-default-config.xml file, usually found in the pool jar
     * 2. jdbc-pool-config.xml file, usually found in your application's classpath
     * 3. Other hardcoded defaults in Pool class
     */
    public PoolConfig() {
        // try to load the default config file, if available from somewhere in the classpath
        loadProperties("jdbc-pool-default-config.xml");
        // try to override with app specific config, if available
        loadProperties("jdbc-pool-config.xml");
    }
     ...
     */
    public PoolConfig(Properties props) throws Exception {
        this();
    ...
    /**
     * Initialize the configuration by loading jdbc-pool-config.xml containing the settings.
     *
     * @param sectionName section to load
     * @throws Exception on parse errors
     */
    public PoolConfig(String sectionName) throws Exception {
        this(PoolConfig.class.getResourceAsStream("/jdbc-pool-config.xml"), checkNotNull(sectionName));
    }
     ...
     * @throws Exception
     */
    public PoolConfig(InputStream xmlConfigFile, String sectionName) throws Exception {
        this();
    ...
    public void setProperties(Properties props) throws Exception {
        ...
        for (Method method : PoolConfig.class.getDeclaredMethods()) {
            String tmp = null;
            ...
            if (method.getParameterTypes().length == 1 && method.getParameterTypes()[0].equals(int.class)) {
                ...
                if (val == null) {
                    val = props.getProperty("jdbc.pool." + tmp);
                }
                if (val != null) {
                    try {
    ...
    @Override
    public PoolConfig clone() throws CloneNotSupportedException {

        PoolConfig clone = (PoolConfig) super.clone();
     ...
     */
    public boolean hasSameConfiguration(PoolConfig that) {
        if (that != null && Objects.equal(this.acquireIncrement, that.getAcquireIncrement())
                ...
                && Objects.equal(this.connectionListener, that.getConnectionListener())
    ...
    /**
     * If true, track statements and close them if application forgot to do so. See also:
     * {@link PoolConfig#detectUnclosedStatements}. Do not set if your connections are managed
     ...
     * the connection was closed. See also: {@link PoolConfig#closeOpenStatements}.
     *
