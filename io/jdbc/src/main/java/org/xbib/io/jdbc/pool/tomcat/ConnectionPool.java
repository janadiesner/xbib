 ...
 * The ConnectionPool uses a {@link PoolProperties} object for storing all the meta information
 * about the connection pool.
 * As the underlying implementation, the connection pool uses {@link java.util.concurrent.BlockingQueue}
 * to store active and idle connections.
 * A custom implementation of a fair {@link FairBlockingQueue} blocking queue is provided with the
...
public class ConnectionPool {
    ...
    private static final Logger log = LogManager.getLogger(ConnectionPool.class.getSimpleName());
     ...
     */
    protected org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool jmxPool = null;
     ...
     * @throws java.sql.SQLException - if the wait times out or a failure occurs creating a
     *                               connection
     */
    ...
    protected void init(PoolConfiguration properties) throws SQLException {
        ...
        for (int i = 0; i < proxies.length; i++) {
            try {
                ...
                if (jmxPool != null) {
                    jmxPool.notify(org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool.NOTIFY_INIT, getStackTrace(x));
                }
                close(true);
    ...
    protected void abandon(PooledConnection con) {
        ...
        try {
            ...
            if (jmxPool != null) {
                jmxPool.notify(org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool.NOTIFY_ABANDON, trace);
            }
    ...
    protected void suspect(PooledConnection con) {
        ...
        try {
            ...
            if (jmxPool != null) {
                jmxPool.notify(org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool.SUSPECT_ABANDONED_NOTIFICATION, trace);
            }
            con.setSuspect(true);
        } finally {
            con.unlock();
    ...
    private PooledConnection borrowConnection(int wait, String username, String password) throws SQLException {
        ...
        while (true) {
            ...
            if (maxWait == 0 && con == null) { //no wait, return one if we have one
                if (jmxPool != null) {
                    jmxPool.notify(org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool.POOL_EMPTY, "Pool empty - no wait.");
                }
            ...
            if (con == null) {
                if ((System.currentTimeMillis() - now) >= maxWait) {
                    if (jmxPool != null) {
                        jmxPool.notify(org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool.POOL_EMPTY, "Pool empty - timeout.");
                    }
                    throw new PoolExhaustedException("[" + Thread.currentThread().getName() + "] " +
     ...
     *
     * @return the object implementing the {@link org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPoolMBean} interface
     */
    public org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool getJmxPool() {
        return jmxPool;
    }
    ...
    protected void createMBean() {
        try {
            jmxPool = new org.xbib.io.jdbc.pool.tomcat.jmx.ConnectionPool(this);
        } catch (Exception x) {
