...
import org.xbib.io.jdbc.pool.bonecp.ConnectionPartition;
 ...
 * Attempts to get a connection from one or more configured partitions.
 */
public class DefaultConnectionStrategy extends AbstractConnectionStrategy {

    public DefaultConnectionStrategy(Pool pool) {
        this.pool = pool;
    }
    ...
    public ConnectionHandle pollConnection() {
        ...
        int partition = (int) (Thread.currentThread().getId() % pool.getPartitionCount());
        ConnectionPartition connectionPartition = pool.getPartitions()[partition];
        result = connectionPartition.getFreeConnections().poll();
        if (result == null) {
            // we ran out of space on this partition, pick another free one
            for (int i = 0; i < this.pool.getPartitionCount(); i++) {
                if (i == partition) {
                    continue; // we already determined it's not here
                }
                result = this.pool.getPartitions()[i].getFreeConnections().poll(); // try our luck with this partition

                if (result != null) {
                    connectionPartition = this.pool.getPartitions()[i]; // we found it here
    ...
    protected Connection getConnectionInternal() throws SQLException {
        ...
        if (result == null) {
            ...
            try {
                result = connectionPartition.getFreeConnections().poll(pool.getConnectionTimeoutInMs(), TimeUnit.MILLISECONDS);
                if (result == null) {
                    if (pool.getConfig().isNullOnConnectionTimeout()) {
                        return null;
                    }
    ...
    public void terminateAllConnections() {
        ...
        try {
            ...
            for (int i = 0; i < this.pool.getPartitionCount(); i++) {
                this.pool.getPartitions()[i].setUnableToCreateMoreTransactions(false); // we can create new ones now, this is an optimization
                List<ConnectionHandle> clist = new LinkedList<ConnectionHandle>();
                this.pool.getPartitions()[i].getFreeConnections().drainTo(clist);
