...
/**
 * Interface to the listener mechanism of a connection lifecycle. Applications
 * will generally want to extend {@link AbstractConnectionListener} instead to provide a
 * default implementation.
 * <p/>
 * Since the class is eventually loaded via reflection, the implementation must
 * provide a public, no argument constructor.
 * <p/>
 * Warning: Be careful to make sure that the listener methods are re-entrant and
...
public interface ConnectionListener {
     ...
     * pool. The connection might be marked as being
     ...
     */
    void onQueryExecuteTimeLimitExceeded(ConnectionHandle conn, Statement statement, String sql, long timeElapsedInNs);
     ...
     */
    void onBeforeStatementExecute(ConnectionHandle conn, StatementHandle statement, String sql);
     ...
     * @param sql       SQL statement about to be executed.
     */
    void onAfterStatementExecute(ConnectionHandle conn, StatementHandle statement, String sql);
     ...
     * <p/>
     * <p/>
     * You may use pool.isConnectionHandleAlive(connection) to verify if the connection is in a usable state again.
     * This method may be called concurrently so any implementation must be thread-safe.
     *
     ...
     * @param connection The handle that triggered this error
     * @param state      the SQLState error code.
     * @param e          Exception that caused us to call this method.
