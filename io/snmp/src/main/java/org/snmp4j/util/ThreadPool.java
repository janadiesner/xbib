...
public class ThreadPool implements WorkerPool {
     ...
     * To stop the execution, a {@link RuntimeException} has to be thrown.
     * The default behavior is to rethrow the interrupted exception wrapped in a {@link RuntimeException}
     ...
     *
     * @param interruptedException the caught InterruptedException.
     * @param task                 the task to should have been executed, but failed to execute (until now) because of a busy pool.
     */
    ...
    public void stop() {
        ...
        synchronized (this) {
            stop = true;
            tms = new ArrayList(taskManagers);
        }
