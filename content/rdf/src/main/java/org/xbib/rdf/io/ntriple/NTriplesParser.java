...
import org.xbib.rdf.io.sink.TripleSink;
...
public final class NTriplesParser implements CharSink {
    ...
    private final static Logger logger = LogManager.getLogger(NTriplesParser.class);
        ...
        for (int pos = start; pos < end; pos++) {
            ...
            if (parsingState == PARSING_OUTSIDE) {
                ...
                if (buffer[pos] == '@' || buffer[pos] == '^') {
                    ...
                    logger.error("unexpected character '" + buffer[pos] + "' after literal");
                }
    ...
    private void processLiteralTypeChar(char[] buffer, int pos) throws IOException {
        if (WHITESPACE.get(buffer[pos])) {
            ...
            if (type.charAt(0) == '@') {
                ...
                logger.error("literal type '" + type + "' can not be parsed");
            }
            parsingState = PARSING_OUTSIDE;
            if (trimSize > 0) {
                finishSentence();
    ...
    private void finishSentence() throws IOException {
        if (waitingForSentenceEnd) {
            ...
            logger.error("unexpected endStream of sentence");
        }
    }

    private void onNonLiteral(String uri) throws IOException {
        if (waitingForSentenceEnd) {
            logger.error("endStream of sentence expected");
        }
        if (subj == null) {
            subj = uri;
        } else if (pred == null) {
    ...
    private void onPlainLiteral(String value, String lang) throws IOException {
        if (subj == null || pred == null) {
            if (waitingForSentenceEnd) {
                ...
                logger.error("literal is not an object");
            }
        }
    ...
    public void endStream() throws IOException {
        if (tokenStartPos != -1 || waitingForSentenceEnd) {
            logger.error("unexpected endStream of stream");
        }
        sink.endStream();
    }
