...
/**
 * Concurrent implementation of {@link SetMultimap} based on the {@link java.util.concurrent.ConcurrentHashMap}
 * implementation.
 * <p>
 * The multimap does not store duplicate key-value pairs. Adding a new key-value pair equal to an existing key-value
 * pair has no effect.
 * <p>
 ...
 * <p>
 * This class obeys the same functional specification as {@link HashMultimap}. However, even though all operations are
 * thread-safe, retrieval operations do <em>not</em> entail locking, and there is <em>not</em> any support for locking
 * the entire structure in a way that prevents all access. This class is fully interoperable with
 * {@link Multimaps#synchronizedSetMultimap(SetMultimap)} in programs that rely on its thread safety but not on its
 ...
 * <tt>putAll</tt> and <tt>clear</tt>, concurrent retrievals may reflect insertion or removal of only some entries.
 * Similarly, Iterators return elements reflecting the state of the structure at some point at or since the creation
 * of the iterator. They do <em>not</em> throw {@link java.util.ConcurrentModificationException}.
 * However, iterators are designed to be used by only one thread at a time.
 */
public final class ConcurrentHashMultimap<K, V> implements ConcurrentSetMultimap<K, V> {
    /*
     ...
     * out as a user-visible Map.Entry, but that it does not support
     * {@link #setValue(java.util.Collection) setValue}. Entries live only
     * through a single iteration, but may not see updates if part of
     * the entry chain is cloned. This satisfies the caveats given in
     * {@link ConcurrentHashMultimap#entries()}.
     * <p>
        ...
        boolean containsValue(Object value) {
            if (elementCount != 0) { // read-volatile
                ...
                for (HashEntry<K, V> aTab : tab) {
                    for (HashEntry<K, V> e = aTab; true; e = e.next) {
                        Set<V> v = e.value;
                        if (v == null) // recheck
                        {
            ...
            try {
                ...
                if (e != null) {
                    ...
                    if (!valuesCollection.isEmpty()) {
                        tab[index] = new HashEntry<K, V>(key, hash, null, valuesCollection);
        ...
        boolean put(K key, int hash, V value) {
            ...
            try {
                ...
                if (e != null) {
                    ...
                    ++modCount;
                    tab[index] = new HashEntry<K, V>(key, hash, null, value);
                    hashEntryCount = incrementedHashEntryCount;
        ...
        boolean remove(Object key, int hash, Object value) {
            ...
            try {
                ...
                if (e != null) {
                    ...
                    boolean removeEntry = (v.size() == 1 && entryContainsValue);
            ...
            public Set<Multiset.Entry<K>> entrySet() {
                ...
                return new AbstractSet<Multiset.Entry<K>>() {
                    ...
                    public Iterator<Multiset.Entry<K>> iterator() {
                        return multisetEntryIterable.iterator();
                    }
    ...
    /**
     * {@inheritDoc} Entries in this set do not support the {@link java.util.Map.Entry#setValue(Object) setValue} operation, since
     ...
     * {@link java.util.ConcurrentModificationException}, and guarantees to traverse elements as they existed upon construction of
    ...
    public boolean equals(Object o) {
        ...
        try {
            for (Entry<K, Collection<V>> e : asMap().entrySet()) {
                K key = e.getKey();
    ...
    public int hashCode() {
        int h = 0;
        for (Entry<K, Collection<V>> kCollectionEntry : asMap().entrySet()) {
            h += kCollectionEntry.hashCode();
        }
        return h;
    }
