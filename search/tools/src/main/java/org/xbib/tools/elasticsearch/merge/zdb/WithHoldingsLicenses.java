...
import org.xbib.tools.elasticsearch.merge.zdb.entities.BibdatLookup;
 ...
 */
public class WithHoldingsLicenses
        extends QueuePipelineExecutor<Boolean, Manifestation, WithHoldingsLicensesPipeline, SearchHitPipelineElement> {

    private final static Logger logger = LoggerFactory.getLogger(WithHoldingsLicenses.class.getSimpleName());
    ...
    private WithHoldingsLicenses service;
    ...
    private WithHoldingsLicenses() {}

    public WithHoldingsLicenses reader(Reader reader) {
        settings = settingsBuilder().loadFromReader(reader).build();
        return this;
    }

    public WithHoldingsLicenses settings(Settings newSettings) {
        settings = newSettings;
        return this;
    }

    public WithHoldingsLicenses writer(Writer writer) {
        return this;
    }


    @Override
    public WithHoldingsLicenses prepare() {
        super.prepare();
    ...
    public WithHoldingsLicenses run() throws Exception {
        ...
        ingest.maxActionsPerBulkRequest(settings.getAsInt("maxBulkActions", 100))
                ...
                .setting(WithHoldingsLicenses.class.getResourceAsStream("transport-client-settings.json"))
                .newClient(targetURI);
        ...
        super.setPipelineProvider(new PipelineProvider<WithHoldingsLicensesPipeline>() {
            int i = 0;

            @Override
            public WithHoldingsLicensesPipeline get() {
                return new WithHoldingsLicensesPipeline(service, i++);
            }
        });
        ...
        // analyze skipped/indexed
        skipped.removeAll(indexed);
        logger.info("skipped minus indexed = {}", skipped.size());

        // log them to file for analysis
        FileWriter w = new FileWriter("notindexed.txt");
    ...
    @Override
    public WithHoldingsLicenses execute() {
        // execute pipelines
        super.execute();
            ...
            for (SearchHit hit : hits) {
                try {
                    ...
                    if (percent != lastpercent && logger.isInfoEnabled()) {
                        logger.info("{}/{} {}% docs={} timelines={} indexed={} skipped={}",
                                count, total, percent,
                                docs.size(), timelines.size(), indexed.size(), skipped.size());
                        for (Pipeline p : getPipelines()) {
