...
public class Manifestation extends MapBasedAnyObject
    ...
    public String id() {
        return id; // !isMicroform ? id : id + "m";
    }

    public String externalID() {
        return externalID; //!isMicroform ? externalID : externalID + "m";
    }
    ...
    public Manifestation inTimeline(boolean b) {
        this.isInTimeline = b;
        return this;
    }
    ...
    private void makeRelations() {
        ...
        for (String rel : relationEntries) {
            ...
            for (Object obj : (List) o) {
                ...
                // external ID = ZDB ID (used for linking, but may collide with GND ID)
    ...
    public void build(XContentBuilder builder, Set<String> visited) throws IOException {
        ...
        if (!isInTimeline()) {
            builder.field("inTimeline", false);
        }
        if (hasIdentifiers()) {
    ...
    public void buildRelated(XContentBuilder builder, Set<String> visited)
        throws IOException {
        SetMultimap<String, Manifestation> relatedManifestions =
                ImmutableSetMultimap.copyOf(getRelatedManifestations());
        if (relatedManifestions != null && !relatedManifestions.isEmpty()) {
            for (String rel : relatedManifestions.keySet()) {
                for (Manifestation mm : relatedManifestions.get(rel)) {
                    mm.build(builder, visited);
                }
            }
        }
    }

    public void buildVolume(XContentBuilder builder, String parentID, Integer date, Set<Holding> holdings)
            throws IOException {
    ...
    public void buildHolding(XContentBuilder builder, String parentID, String isil, Set<Holding> holdings)
        throws IOException {
        if (holdings == null || holdings.isEmpty()) {
            return;
        }
    ...
    public static Comparator<Manifestation> getComparator() {
        return idComparator;
    }
