 ...
 *
 */
public class GND implements TripleListener {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            final GND gnd = new GND(elasticsearch, index, type, mock);

            IRI id = IRI.builder().scheme("http").host("d-nb.info").path("/gnd/").build();
            TurtleReader reader = new TurtleReader(id);
            reader.setTripleListener(gnd);
            reader.parse(in);
            gnd.close();
            logger.info("done, indexed resources have " + gnd.getTripleCounter() + " triples.");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
        ...
        GND(String esURI, String index, String type, boolean mock) throws IOException {
