...
public class ArticleDB extends AbstractPipeline<CounterElement> {
    ...
    private final static CounterElement inputCounter = new CounterElement().set(new AtomicLong(0L));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            SimplePipelineExecutor service = new SimplePipelineExecutor()
                    .concurrency((Integer) options.valueOf("threads"))
                    .provider(
    ...
    public CounterElement next() {
        ...
        try {
            while (!done) {
                ...
                inputCounter.get().incrementAndGet();
            }
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            done = true;
