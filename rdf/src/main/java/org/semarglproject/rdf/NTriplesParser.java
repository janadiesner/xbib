 ...
 */
public final class NTriplesParser extends AbstractPipe<TripleSink> implements CharSink {
    ...
    private void finishSentence() throws IOException {
        if (waitingForSentenceEnd) {
            ...
            logger.error("unexpected end of sentence");
        }
    }

    private void onNonLiteral(String uri) throws IOException {
        if (waitingForSentenceEnd) {
            logger.error("end of sentence expected");
        }
        if (subj == null) {
            subj = uri;
        } else if (pred == null) {
