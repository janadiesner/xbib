 ...
 * ---         | ---                               | ---
 * 0           | {@link org.mapdb.StoreDirect#HEADER}        | File header
 * 1           | {@link org.mapdb.StoreDirect#IO_INDEX_SIZE} | Allocated file size of index file in bytes.
 * 2           | {@link org.mapdb.StoreDirect#IO_PHYS_SIZE}  | Allocated file size of physical file in bytes.
 * 3           | {@link org.mapdb.StoreDirect#IO_FREE_SIZE}  | Space occupied by free records in physical file in bytes.
 * 4           | {@link org.mapdb.StoreDirect#IO_INDEX_SUM}  | Checksum of all Index file headers. Checks if store was closed correctly
 * 5..9        |                                   | Reserved for future use
 * 10..14      |                                   | For usage by user
 * 15          | {@link org.mapdb.StoreDirect#IO_FREE_RECID} |Long Stack of deleted recids, those will be reused and returned by {@link Engine#put(Object, Serializer)}
 ...
 * 4112        | {@link org.mapdb.StoreDirect#IO_USER_START} |Record size and offset in physical file for recid=1
 ...
 * 16-59    |`val&{@link org.mapdb.StoreDirect#MASK_OFFSET}`      | physical offset
 * 60       |`val&{@link org.mapdb.StoreDirect#MASK_LINKED}!=0`   | linked record flag
 * 61       |`val&{@link org.mapdb.StoreDirect#MASK_DISCARD}!=0`  | to be discarded while storage is offline flag
 * 62       |`val&{@link org.mapdb.StoreDirect#MASK_ARCHIVE}!=0`  | record modified since last backup flag
 * 63       |                                           | not used yet
 * <p/>
...
public class StoreDirect extends Store {
    ...
    protected void checkHeaders() {
        if (index.getLong(0) != HEADER || phys.getLong(0) != HEADER) {
    ...
    public <A> void delete(long recid, Serializer<A> serializer) {
        ...
        try {
            ...
            if (!spaceReclaimTrack) {
                return; //free space is not tracked, so do not mark stuff as free
            }
