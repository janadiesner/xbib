 ...
 */
public final class ZDBFromMARC extends Feeder {

    private final static Logger logger = LoggerFactory.getLogger(ZDBFromMARC.class.getName());
    ...
    protected PipelineProvider<Pipeline> pipelineProvider() {
        return new PipelineProvider<Pipeline>() {
            @Override
            public Pipeline get() {
                return new ZDBFromMARC();
            }
        };
    }

    @Override
    protected ZDBFromMARC prepare(Ingest output) {
        return this;
    }
        ...
        final MARCElementMapper mapper = new MARCElementMapper(settings.get("elements"))
                ...
                .start(new MARCElementBuilderFactory() {
                    public MARCElementBuilder newBuilder() {
                        return new MARCElementBuilder().addOutput(new MyElementOutput());
                    }
                });
    ...
    private class MyElementOutput extends CountableElementOutput<ResourceContext, Resource> {

        @Override
        public void output(ResourceContext context, ContentBuilder contentBuilder) throws IOException {
            IRI iri = context.resource().id();
            context.resource().id(IRI.builder().scheme("http").host(settings.get("index")).query(settings.get("type"))
