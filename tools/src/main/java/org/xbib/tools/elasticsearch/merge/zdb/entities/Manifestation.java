...
public class Manifestation extends MapBasedAnyObject
    ...
    public Manifestation(Map<String, Object> m) {
        ...
        this.relatedVolumes = HashMultimap.create();
    ...
    private Map<String,Object> makeIdentifiers() {
        ...
        // get CODEN for better article linking
    ...
    private void makeRelations() {
        ...
        // this manifestation is in the time line iff it has a successor,
        // but if there is a transient edition, the chance is there is
        // no successor (which is a cataloging mistake)
        // Example: ZDB ID 570215x
        this.isInTimeline = hasSuccessor && !hasTransient;
    }
    ...
    public void build(XContentBuilder builder, Set<String> visited) throws IOException {
        ...
        if (map != null && !map.isEmpty()) {
            ...
            if (!relatedHoldings.isEmpty()) {
                for (String institutionID : relatedHoldings.keySet()) {
                    builder.startObject()
                        .field("@id", institutionID)
                        .field("@type", "Institution")
                        .field("@label", "heldBy")
                        .endObject();
                }
            }
            if (!relatedVolumes.isEmpty()) {
                for (Integer date : relatedVolumes.keySet()) {
                    builder.startObject()
                            .field("@id", date)
                            .field("@type", "Volume")
                            .field("@label", "hasVolume")
                            .endObject();
                }
            }
            builder.endArray();
    ...
    public void buildRelated(XContentBuilder builder, Set<String> visited)
        ...
        if (relatedManifestions != null && !relatedManifestions.isEmpty()) {
            builder.startObject().startArray("related");
