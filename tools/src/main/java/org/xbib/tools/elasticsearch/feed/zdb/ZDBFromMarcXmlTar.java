...
public final class ZDBFromMarcXmlTar extends Feeder {
        ...
        final MARCElementMapper mapper = new MARCElementMapper(settings.get("elements"))
                ...
                .start(new MARCElementBuilderFactory() {
                    public MARCElementBuilder newBuilder() {
                        return new MARCElementBuilder().addOutput(new OurElementOutput());
                    }
                });
    ...
    private class OurElementOutput extends CountableElementOutput<ResourceContext, Resource> {
        ...
        public void output(ResourceContext context, ContentBuilder contentBuilder) throws IOException {
            IRI id = context.resource().id();
            context.resource().id(IRI.builder().scheme("http").host(settings.get("index")).query(settings.get("type")).fragment(id.getFragment()).build());
